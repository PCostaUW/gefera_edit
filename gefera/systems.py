...
class System:
    ...
    def lightcurve(self, t, u1, u2, r1, r2, grad=False, integrate=None, dt=None):
        ...
        if grad:
            ...
            dfde1cos = (grad['w1']-dfdt0*dt0dw(tp,ep,pp,wp)-dfdb1*dbdw(ap,ip,ep,wp)-dfdT*dTdw(pp,ap,wp,ep,ip)-(grad['e1']-dfdT*dTde(pp,ap,wp,ep,ip)-dfdt0*dt0de(tp,ep,pp,wp)-dfdb1*dbde(ap,ip,ep,wp))*desinwdw(ep,wp)/desinwde(ep,wp)) / (1-decoswde(ep,wp)/desinwde(ep,wp)) 
