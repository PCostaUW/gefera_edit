...
def i1(b1,p1,T,ecosw,esinw):
    ...
    return np.arccos(b1/np.sqrt((1-b1**2)/(np.sin(Pi*T/p1*(1+esinw)**2/(1-ecc(ecosw,esinw)**2)**(3/2))**2)+b1**2))
...
def t1(t0,p,ecosw,esinw):
    ...
    Args:
        ...
        esinw: component of eccentricity
    '''
    return t0+p*np.sqrt(1-ecc(ecosw,esinw)**2)/(2*Pi)*(-ecosw/(1-esinw) - 2/np.sqrt(1-ecc(ecosw,esinw)**2)*atan2(np.sqrt(1-ecc(ecosw,esinw)**2)*np.tan(0.75*Pi-0.5*omega(ecosw,esinw)),(1+ecc(ecosw,esinw))))
